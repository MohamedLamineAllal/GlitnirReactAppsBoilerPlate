...
export interface IIndicatorValue {
    time: Time,
    value: number,
    mark: boolean | SeriesMarker<Time>
}
...
function createUpdateModifier(
    ...
    maxIndicatorValueRef: RefObject<number>
    ...
    return (data: IFeedData) => {
        ...
        if (seriesRef.current) {
            ...
            for (const value of data.indicatorValues) {
                seriesRef.current.whalesValueLineSerie.update(value);
                // update the markers
            }

            // ___________________________ updating feed
        ...
        if (
            ...
            for (const d of data) {
                ...
                for (const value of d.values) {
                    ...
                    if (
                        maxIndicatorValueRef.current as number < d.value ||
                        !maxIndicatorValueRef.current
                    ) {
                        maxIndicatorValueRef.current = d.value;
                    }
    ...
    useEffect(() => {
        ...
        function crosshairMoveHandler({
            ...
            if (
                ...
                valueLineCurrentValue.options()
                valueLabel.style.bottom = `${(price as number * 1 / 4 * (chartOptions.height || 800) / maxIndicatorValueRef.current) + 10}px`;
                console.log("maxindcator value ref = " + maxIndicatorValueRef.current)
                valueLabel.innerHTML = `${price}`;
