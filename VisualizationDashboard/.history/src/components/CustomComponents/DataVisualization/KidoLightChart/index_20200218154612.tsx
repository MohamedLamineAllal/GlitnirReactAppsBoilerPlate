...
function createSetModifier(
    ...
    return (data: IFeedData[]) => {
        ...
        if (seriesRef.current) {
            ...
            console.log(data)
            // ___________________________ updating the candles
            const markers: Omit<IFeedData, 'tickCandles'> = {
                closeSigNoDoubleSignals: [],
                firstSigNoDoubleSignals: [],
                mainSignals: []
            };

            for (const oneFeedData of data) {
                // __________________________ update tick candles
                for (const tickCandle of oneFeedData.tickCandles) {
                    seriesRef.current.tickCandles.update(tickCandle);
                }

                // _______________________ accumulate markers
                for (const markerName of CHART_MARKERS_NAMES) {
                    markers[markerName] = [
                        ...markers[markerName],
                        ...oneFeedData[markerName]
                    ];
                }
            }
